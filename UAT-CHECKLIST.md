...
- **Config-per-file rule (D-06):** one checklist row per `@ConfigEntity`-annotated class, never one
  row per key. This module ships exactly one yml resource (`config.yml`, all 8 declared keys
...
| ultibot.bot.reload | An OP sender or one holding `ultibot.admin`; `config.yml` hand-edited to a distinguishable value (e.g. `max-total-bots`) BEFORE running this row's Steps | Run `/bot reload`, then attempt to observe the edited value's effect (e.g. spawn bots up to the new `max-total-bots` and confirm the OLD limit, not the new one, is what actually applies) | Chat line reads `Configuration reloaded` (green) — but the edited config value's effect does NOT change: `onReload` never re-reads `config.yml` at all (UltiKits/UltiBot#16), so this row's whole point is confirming the success message is emitted despite nothing being reloaded | server | |
...
| ultibot.bot.action.neg-no-op-types | A named bot active | Run `/bot action <name> ATTACK 20` (also try `MINE` and `LOOK_AT_NEAREST`) | The command still reports `Bot <name> started action: ATTACK` (green, since these are valid enum values) but — separately from the scheduling defect above — even manually invoking the underlying action once (via a debugger, or once UltiKits/UltiBot#17 is fixed) would produce NO observable effect: `ActionServiceImpl#performAction`'s `switch` falls through all three cases with an empty body | server | |
| ultibot.bot.stop | A named bot with at least one active repeating action started via `ultibot.bot.action` above | Run `/bot stop <name>` | Chat line reads `Bot <name> stopped all actions` (green); the ticker's own `isRunning()` would now report `false` if it were ever inspectable — though since UltiKits/UltiBot#17 means the ticker was never actually ticking in the first place, this row observes only the chat confirmation and the state-tracking map being cleared, not a behavioral change | server | |
...
| ultibot.bot.skin | A named bot active; a real, existing Mojang player name (e.g. `Notch`) NOT already cached by a prior fetch in this session — this row is timed specifically to observe the main-thread block described in UltiKits/UltiBot#18 | Run `/bot skin <name> Notch`, and independently confirm the server's tick rate / other players' actions during the call | Chat line reads `Bot <name> skin changed to Notch` (green) once the fetch completes; observe that OTHER players' movement/interactions freeze for up to several seconds while the two synchronous Mojang HTTP calls are in flight — confirming UltiKits/UltiBot#18. A second `/bot skin <name> Notch` call for the SAME name completes near-instantly (cache hit), which is the contrast that proves the delay is network-bound, not command-dispatch overhead | server | |
...
| ultibot.bot.click.neg-no-inventory | A named bot with NO open inventory | Run `/bot click <name> 0` | Chat line reads `Bot <name> has no open inventory` (red) | server | |
...
| ultibot.bot.inv.neg-no-inventory | A named bot with NO open inventory | Run `/bot inv <name>` | Chat line reads `Bot <name> has no open inventory` (red) | server | |
...
| ultibot.bot.macro-play | A macro named `test-macro` previously saved (start a recording via `/bot macro record` against any bot, then end it with `/bot macro stop`); a named bot active | Run `/bot macro play <name> test-macro` | Chat line reads `Playing macro: test-macro` (green) — but NONE of the macro's recorded actions are ever actually replayed against the bot: `onMacroPlay` only looks up the macro's entry list to confirm it exists and prints this confirmation; it never iterates the entries to re-invoke any action (UltiKits/UltiBot#17's scheduling gap applies here too — even if it did iterate them, the same dead `tickAll()` path would apply) | server | |
...
| ultibot.bot.macro-record | A named bot active, not currently recording a macro | Run `/bot macro record <name> test-macro`, then run one or more `/bot action` commands against the SAME bot while recording is active | Chat line reads `Started recording macro for bot <name>: test-macro` (green) — but the SUBSEQUENT `/bot action` invocations are NOT actually captured into the macro's entry list: `MacroServiceImpl#recordAction` is the only method that appends to a `RecordingSession`'s entries, and it has ZERO callers anywhere in this module's source (`BotCommands#onAction`/`ActionServiceImpl#startRepeatingAction` never call it) — recording a macro captures nothing regardless of what actions run while "recording" is active | server | |
...
| ultibot.event.death-respawn.neg-disabled | `auto-respawn: false`; same lethal setup as the row above | Cause the bot to die | The bot is NOT automatically respawned; `/bot list` no longer shows it as active (or shows it in whatever "dead" state a `ServerPlayer` occupies without a respawn call) | server | |

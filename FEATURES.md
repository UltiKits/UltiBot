...
| ultibot.bot.skin | Fetch a real player's skin from Mojang's API (two synchronous, BLOCKING HTTP calls, up to 5s timeout each), caching the result by player name — but the fetched value/signature is NEVER applied to the bot's `GameProfile` anywhere in this module's source; the command reports success regardless. **The command's own headline behavior — actually changing the bot's appearance — does not exist** (UltiKits/UltiBot#21). Separately, the fetch itself runs on whatever thread invoked the command — for an in-game player, the SERVER MAIN THREAD, which the framework's own Common Gotcha #1 states must never block on I/O; a cold (uncached) fetch can freeze the entire server for up to ~10 seconds (UltiKits/UltiBot#18) | command | `/bot skin <name> <skinName>` | ultibot.use | both | admin | detailed | ultibot-core: BotCommands#onSkin |
...
| ultibot.bot.inv | Print the contents of a named bot's currently open inventory (slot index, display name or material, stack size) to the sender. Its own "no open inventory" refusal branch is DEAD CODE for a live bot, for the identical reason as `ultibot.bot.click` above (UltiKits/UltiBot#22) — a bare `/bot inv` against any live bot always prints SOMETHING, at minimum that bot's own inventory contents | command | `/bot inv <name>` | ultibot.use | both | internal | brief | ultibot-core: BotCommands#onInventory |
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultibot.config.config.allow-chunk-loading | Declared as the switch controlling whether bots keep their chunks loaded; `BotConfig#isAllowChunkLoading()` has **zero readers anywhere in this module's source** outside its own field declaration — no code anywhere adds a chunk ticket, registers a keep-loaded region, or otherwise reads this key at all. Filed as UltiKits/UltiBot#19 (grouped with the module's other declared-but-unread config keys) | config | `config.yml: allow-chunk-loading (default: false, no effect, see UltiKits/UltiBot#19)` | n/a | n/a | admin | none | BotConfig#isAllowChunkLoading (declared, never read outside this class) |
